...
// 画单个像素（内部使用）
static void font_draw_pixel(unsigned char *fbp, int line_len, int x, int y, unsigned int color)
{
    if (x < 0 || y < 0 || x >= 1024 || y >= 600) // 适配你的1024x600屏幕，超出边界不画
        return;
...
// 在(x,y)位置画一个字符
// 参数：显存指针, 行字节数, 起始x坐标, 起始y坐标, 字符, 颜色
static void draw_char(unsigned char *fbp, int line_len, int x, int y, char ch, unsigned int color)
{
    if (ch < ' ' || ch > '~')
        ch = ' '; // 超出范围显示空格
    int idx = ch - ' '; // 计算字模索引
    
    for (int row = 0; row < FONT_8X16_HEIGHT; row++) {
        unsigned char line_data = font_8x16[idx][row];
        for (int col = 0; col < FONT_8X16_WIDTH; col++) {
            if (line_data & (0x80 >> col)) { // 高位在前，从左到右绘制
                font_draw_pixel(fbp, line_len, x + col, y + row, color);
            }
        }
    }
}
...
// 在(x,y)位置画一行字符串
// 参数：显存指针, 行字节数, 起始x坐标, 起始y坐标, 字符串, 颜色
static void draw_string(unsigned char *fbp, int line_len, int x, int y, const char *str, unsigned int color)
{
    int cx = x;
    while (*str) {
        draw_char(fbp, line_len, cx, y, *str, color);
        cx += FONT_8X16_WIDTH; // 每个字符占8像素宽度
        str++;
    }
}
